package com.whf.convert.tree;


import com.whf.convert.tree.interfaces.TreeFid;
import com.whf.convert.tree.interfaces.TreeId;
...
public class ConvertTree<T> {
     ...
     * List<TreeNode> result= convertTree.getForest(数据List，id字段名，父id字段名);
     * 2.2注解使用
     *
     * 将@TreeId和@TreeFid分别注解在T的id字段和父id字段上
     * ConvertTree convertTree = new ConvertTree<>();
     * List<TreeNode> result= convertTree.getForest(数据List);
     */
